...
CPU_TEST( 61, ADC, IndirectX, "small.json" );
...
std::string CPUTest::GetCPUStateString( const json& jsonData, const std::string& state )
{
        ...
        [&]( const std::string& label, const std::string& expected, const std::string& actual )
    {
        ...
        {
            std::stringstream ss;
            int               intValue = std::stoi( value );  // Convert string to integer
...
void CPUTest::RunTestCase( const json& testCase )  // NOLINT
{
    ...
    std::string testName = testCase["name"];
    if ( verbose )
    {
    ...
    if ( cpu.GetPC() != u16( testCase["initial"]["pc"] ) )
    {
        testFailed = true;
        errorMessages << "PC\n";
    }
    if ( cpu.GetA() != testCase["initial"]["a"] )
    {
        testFailed = true;
        errorMessages << " A \n";
    }
    if ( cpu.GetX() != testCase["initial"]["x"] )
    {
        testFailed = true;
        errorMessages << " X \n";
    }
    if ( cpu.GetY() != testCase["initial"]["y"] )
    {
        testFailed = true;
        errorMessages << " Y \n";
    }
    if ( cpu.GetS() != testCase["initial"]["s"] )
    {
        testFailed = true;
        errorMessages << " S \n";
    }
    if ( cpu.GetP() != testCase["initial"]["p"] )
    {
        testFailed = true;
        errorMessages << " P \n";
    }
    for ( const auto& ramEntry : testCase["initial"]["ram"] )
    {
        ...
        {
            testFailed = true;
            errorMessages << " RAM  at address " << std::hex << address << "\n";
        }
    }

    // Execute final state checks
    if ( cpu.GetPC() != u16( testCase["final"]["pc"] ) )
    {
        testFailed = true;
        errorMessages << "Final PC \n";
    }
    if ( cpu.GetA() != u8( testCase["final"]["a"] ) )
    {
        testFailed = true;
        errorMessages << "Final A \n";
    }
    if ( cpu.GetX() != u8( testCase["final"]["x"] ) )
    {
        testFailed = true;
        errorMessages << "Final X \n";
    }
    if ( cpu.GetY() != u8( testCase["final"]["y"] ) )
    {
        testFailed = true;
        errorMessages << "Final Y \n";
    }
    if ( cpu.GetS() != u8( testCase["final"]["s"] ) )
    {
        testFailed = true;
        errorMessages << "Final S \n";
    }
    if ( cpu.GetP() != u8( testCase["final"]["p"] ) )
    {
        testFailed = true;
        errorMessages << "Final P \n";
    }
    for ( const auto& ramEntry : testCase["final"]["ram"] )
    {
        ...
        {
            testFailed = true;
            errorMessages << "Final RAM mismatch at address " << std::hex << address << "\n";
        }
    }

    // Print initial and final states only if the test failed
    if ( testFailed )
    {
        std::string initialState = GetCPUStateString( testCase, "initial" );
        std::string finalState = GetCPUStateString( testCase, "final" );

        std::cerr << "Test failed with the following mismatches:\n"
                  << errorMessages.str() << '\n'
                  << initialState << '\n'
                  << finalState << '\n';
